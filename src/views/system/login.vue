<template>
  <div class="login-wrap">
    <video src="../../assets/img/yaNVMVgY2r.mp4" style="width: 100%;height: 100%;object-fit: cover;position: absolute;top: 0;left: 0;" autoplay="autoplay" loop="loop" muted="muted"></video>
    <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="demo-ruleForm login-container">
      <!-- <h3 class="title">用户登录</h3> -->
      <div class="system-name">
        <img src="../../assets/img/logo.png" style="width: 36px;height: 36px;vertical-align: top;" alt="">
        <span style="font-size: 21px;font-weight: 800;margin-left: 15px;">空地一体化智能水环境监测平台</span>
      </div>
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item prop="code">
            <el-input type="text" v-model="ruleForm.code" size="large" auto-complete="off" placeholder="图形验证码" @keyup.enter.native="submitForm('ruleForm')" ></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12" class="code-box">
          <!-- <img :src="ruleForm.codeimg" alt="" class="codeimg" @click="getcode()"> -->
          <canvas id="canvas" width="120" height="40" style="margin-left: 20px;border-radius: 3px;cursor: pointer;" @click="createCode"></canvas>
        </el-col>
      </el-row>
...
/*   setup() {
  ...
  methods: {
    // 获取用户名密码
    submitForm() {
      console.log(this.ruleForm.code.toLowerCase(),this.ruleForm.codeimg)
      if(this.ruleForm.password === 'admin' && this.ruleForm.username === 'admin' && this.ruleForm.code.toLowerCase() === this.ruleForm.codeimg) {
        this.$router.push({ path: '/reciprocal' })
          console.log('111')
      }else {
    ...
    createCode() {
      ...
      for(let i=0;i<5;i++) {
        ctx.beginPath()
        ctx.moveTo(this.rn(0,120),this.rn(0,40))
        ctx.lineTo(this.rn(0,120),this.rn(0,40))
      ...
      for(let i=0;i<40;i++) {
        ctx.beginPath()
        ctx.arc(this.rn(0,120),this.rn(0,40),1,0,2*Math.PI)
        ctx.closePath()
        ctx.fillStyle = this.rc(150,200)
        ctx.fill()
      }
      console.log(result.toLowerCase())
      this.ruleForm.codeimg = result.toLowerCase()
    }
  }
}
</script>
...
.login-wrap {
  ...
  /* background-image: url(../../assets/img/yaNVMVgY2r.mp4); */
...
.login-container  {
  ...
  border: 3px solid rgba(25,81,127);
  color: #fff;
  text-align: left;
  box-shadow: 0 0 20px 2px rgba(0, 0, 0, 0.1);
  animation: login-container 2s 1;
